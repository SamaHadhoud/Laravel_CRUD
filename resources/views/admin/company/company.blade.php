<x-app-layout>
    <a href="/redirect" class="inline-block text-black ml-4 mb-4"><i class="fa-solid fa-arrow-left"></i> Back
    </a>
    <div class="mx-4">
        <div class = 'bg-gray-50 border border-gray-200 rounded p-10'>
        <div class="flex flex-col items-center justify-center text-center">
          <img class="w-48 mr-6 mb-6"
            src="{{$company->logo ? asset('logo/' . $company->logo) : asset('/images/no-image.png')}}" />

          <h3 class="text-2xl mb-2">
            {{ $company->name}}
          </h3>

          <div class="text-lg my-4">
            <i class="fa-solid fa-location-dot"></i> {{$company->location}}
          </div>
          <div class="border border-gray-200 w-full mb-6"></div>
          <div>
            <h3 class="text-3xl font-bold mb-4"> Description</h3>
            <div class="text-lg space-y-6">
              {{$company->description}}

              <a href="mailto:{{$company->email}}"
                class="block bg-laravel text-white mt-6 py-2 rounded-xl hover:opacity-80"><i
                  class="fa-solid fa-envelope"></i>
                Contact Employer</a>

              <a href="{{$company->website}}" target="_blank"
                class="block bg-black text-white py-2 rounded-xl hover:opacity-80"><i class="fa-solid fa-globe"></i>
                Visit Website</a>
            </div>
          </div>
          <div class= "d-flex m-3">
          <a href="/admin/company/{{$company->id}}/edit">
            <i class="fa-solid fa-pencil"></i> Edit
          </a>
          <form method="POST" action="/admin/company/{{$company->id}}" onsubmit="return confirm('Are you sure?');">
            @csrf
            @method('DELETE')
            <button class="text-red-500"><i class="fa-solid fa-trash"></i> Delete</button>
          </form>
          <div>
        </div>
    </div>


    </div>
</x-app-layout>
